...
export function sha256Fast(ascii: string): Uint8Array {
  const len = ascii.length;
  const bitLen = len * 8;
  const padLen = ((len + 8) >> 6) + 1;
  const blockCount = padLen << 4;
  const words = new Uint32Array(blockCount);

  for (let i = 0; i < len; i++) {
    words[i >> 2] |= (ascii.charCodeAt(i) & 0xff) << (24 - (i % 4) * 8);
  }
  words[len >> 2] |= 0x80 << (24 - (len % 4) * 8);
  words[blockCount - 1] = bitLen;
...
export async function solvePoW(
  challenge: string,
  difficulty: number,
  maxIterations: number = 50_000_000,
): Promise<number | null> {
  let nonce = 0;
  const prefix = challenge + ":";

  while (nonce < maxIterations) {
    const hash = sha256Fast(prefix + nonce);
